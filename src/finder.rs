...
impl Generator {
    ...
    fn create(crate_path: OsString, yaml_doc: &Yaml) -> Result<Generator, Error> {
    ...
    P: AsRef<Path> + AsRef<OsStr>,
{
    ...
    match list_dep_root_dirs(root_crate_path) {
        Err(e) => vec![Err(e)],
        Ok(dep_root_dirs) => {
            // Find all roots that contain cargo_generators.yaml's.
            find_dirs_with_yaml_files(&dep_root_dirs).map(|dep_root_dir| {
                let parsed_yaml = parse_yaml(dep_root_dir.join("cargo_generators.yaml"));
                Generator.create(dep_root_dir, parsed_yaml)
            })
            // // Parse each yaml
...
fn find_dirs_with_yaml_files(paths: &[PathBuf]) -> Filter<PathBuf> {
    paths
        .iter()
        .filter(|path| path.join("cargo_generators.yaml").is_file())
}

fn parse_yaml(path: PathBuf) -> Result<Yaml, Error> {
    ...
    match YamlLoader::load_from_str(&yaml_str) {
        ...
        Ok(yamls) => match yamls[0].as_vec() {
            None => Err(err_msg("A generators YAML file is not an array")),
            Some(yamls) => yamls.iter().map(|yaml| Ok(yaml.clone())),
        },
    }
}
...
//     yamls
//         .into_iter()
//         .map(|res| res.and_then(|yaml| Generator::create(&yaml)))
//         .collect()
// }
...
mod from_yaml_test {
    ...
    fn it_parses_generator_name_from_yaml() {
        ...
        assert_eq!(
            "a.x",
            Generator::create(OsString::new(), &yaml[0]).unwrap().name
        );
    }
    ...
    fn it_fails_if_generator_name_is_missing() {
        ...
        assert!(Generator::create(OsString::new(), &yaml[0]).is_err());
    }
    ...
    fn it_parses_generator_factory_from_yaml() {
        ...
        assert_eq!(
            "f",
            Generator::create(OsString::new(), &yaml[0])
                .unwrap()
                .factory
        );
    }
...
mod find_all_test {
    ...
    #[test]
    fn it_finds_generators_in_the_current_dir() {
        ...
        assert_eq!(
            ...
            find_all(crate_dir.path())
                .into_iter()
                .map(|res| {
                    res.map(|generator| generator.name)
    ...
    #[test]
    fn it_finds_generators_in_a_dependency() {
    ...
    #[test]
    fn it_finds_generators_in_a_member_package() {
    ...
    fn it_fails_on_invalid_yaml() {
        let crate_dir = create_empty_crate("cargo-gen-test").unwrap();
        FileHelper::new(false)
            .create_file(crate_dir.path().join("cargo_generators.yaml"), "[{]}")
            .unwrap();
        assert_eq!(
            vec!["ERROR"],
            find_all(crate_dir.path())
